...
Definition oddmembers (l : natlist) : natlist := filter oddb l.
...
Definition nbeq_nat (x:nat) (y:nat) : bool := negb (beq_nat x y).

Definition remove_all (v:nat) (s:bag) : bag := filter (nbeq_nat v) s.
...
Lemma app_nil_r : forall xs : bag, xs ++ [ ] = xs.
Proof. intros. induction xs; [ reflexivity | 
                               simpl; rewrite -> IHxs ]; 
               reflexivity.
Qed.

Theorem countPreserved : forall xs ys : bag, forall v : nat, 
                           count v xs + count v ys = count v (xs ++ ys).
Proof.
  intros. induction xs; [ reflexivity | ].
  replace ((n :: xs) ++ ys) with (n :: (xs ++ ys)).
  assert (H: forall zs : bag, forall y z : nat, count y (z :: zs) = count y [z] + count y zs).
    intros. simpl. induction (beq_nat y z); reflexivity.
  rewrite -> H.
  assert (H' : count v (n :: xs ++ ys) = count v [n] + count v (xs ++ ys)).
    rewrite -> H. reflexivity.
  rewrite -> H'. rewrite <- plus_assoc. rewrite -> IHxs. reflexivity.
  Case "Proof of replace"; reflexivity.
Qed.       
(** [] *)
...
Proof. intros; destruct l as [| n l']; reflexivity. Qed.
...
Lemma rev_snoc_elim : forall l : natlist, forall x : nat,
...
Proof.
  ...
  rewrite -> rev_snoc_elim. rewrite -> IHl. reflexivity.
Qed.
...
Proof.
  ...
  rewrite -> IHl1. rewrite -> snoc_append. rewrite -> snoc_append. rewrite -> app_assoc. 
  reflexivity.
Qed.
...
Proof.
  ...
  assert (H: forall x : nat, forall xs : natlist, nonzeros (x :: xs) = nonzeros [x] ++ nonzeros xs).
  Case "Proof of assertion". intros. destruct x; reflexivity.
  rewrite -> H. replace ((n :: l1) ++ l2) with (n :: l1 ++ l2).
  rewrite -> H. rewrite -> IHl1. rewrite -> app_assoc. reflexivity.
  Case "Proof of replacement". reflexivity.
Qed.
(** [] *)
...
Proof.
  intros; induction l; [ reflexivity | simpl ].
  rewrite <- beq_nat_refl. rewrite <- IHl; reflexivity.
Qed.
(** [] *)
...
Proof. intros; induction s; [ reflexivity | ].
       destruct n; simpl; [ rewrite -> ble_n_Sn | rewrite -> IHs ]; reflexivity.
Qed.
...
Theorem rev_surjective : forall l1 l2 : natlist, l1 = l2 -> rev l1 = rev l2. 
Proof. intros; induction l1; rewrite -> H; reflexivity. Qed.

Theorem rev_injective : forall l1 l2 : natlist, rev l1 = rev l2 -> l1 = l2.
Proof. intros. apply rev_surjective in H. rewrite -> rev_involutive in H.
       rewrite -> H. rewrite -> rev_involutive. reflexivity.
Qed.

Theorem rev_bijective : forall l1 l2 : natlist, rev l1 = rev l2 <-> l1 = l2.
Proof. split; intros. 
       apply rev_injective in H; exact H.
       apply rev_surjective in H; exact H.
Qed.
...
Proof. intros; destruct d; simpl; rewrite <- beq_nat_refl; reflexivity. Qed.
