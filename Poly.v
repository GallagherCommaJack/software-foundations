...
Definition list123' := cons nat 1 (cons nat 2 (cons nat 3 (nil _))).
...
Theorem rev_snoc_elim : forall (X : Type) (l : list X) (x : X), 
                          rev (snoc l x) = x :: rev l.
Proof. intros. induction l. reflexivity. 
       simpl. rewrite -> IHl. reflexivity.
Qed.

Lemma map_snoc_elim : forall (X Y : Type) (f : X -> Y) (l : list X) (x : X),
...
Proof. intros; induction l; [reflexivity| ].
       simpl. rewrite -> map_snoc_elim;  rewrite <- IHl. reflexivity.
Qed.
...
Proof.
  intros X l. unfold fold_length. induction l; [reflexivity|simpl].
...
Proof.
  intros. unfold fold_map. induction l; [reflexivity | simpl].
  rewrite -> IHl. reflexivity.
Qed.  
(** [] *)
